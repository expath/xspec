...
echo %XSPEC%
java -cp "%CP%" net.sf.saxon.Transform -o:%TEST_STYLESHEET% -s:%XSPEC% -xsl:generate-xspec-tests.xsl
echo. 
...
GOTO endif4
:coverage2 
 java -cp "%CP%" net.sf.saxon.Transform -l:on -o:%COVERAGE_HTML% -s:%COVERAGE_XML% -xsl:coverage-report.xsl "tests=%XSPEC%"
 %COVERAGE_HTML% 
:endif4
